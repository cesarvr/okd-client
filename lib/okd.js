...
function get_okd_api(credentials) {
    return new Promise((resolve, reject) => {
        let {token, cluster, namespace} = credentials

        if(token !== undefined) {
            console.log('using token->', token)
            api = builder(cluster, token)
            api.namespace('testing-2').pod.all().then(nss => console.log('ns->', nss) ).then(ok => resolve(api))
        }else     
            login(credentials).then(api => resolve(api)) 
    })
}

module.exports = get_okd_api 
