...
const WATCH_URLS = {
    ...
    rs:     'https://$ENDPOINT/api/v1/watch/namespaces/$NAMESPACE/replicationcontrollers',
}
...
function make_watch(tmpl_url, okd) {
    return (target, cb) => {
        let cli = okd.config(({ namespace, cluster, token, strictSSL }) => {
...
function extend(obj) {
    ...
    obj['is'].on_new  = onNew(obj['is'])
    obj['pod'].on_new = onNew(obj['pod'])
}
